  ...
  joincarpoolserv: async (body) => {
    try {
      if (
        ...
        let commuter_id_new = findrides[0]["commuter_id"];
        let price_comm = findrides[0]["price"];
        let uid_body = body.uid;
        price_comm.push(Number(price).toFixed);
        console.log('price_comm',price_comm);
        let seats_avaialble = Number(findrides[0]["seat_available"]) - 1;
        commuter_id_new.push(Number(body.uid));
        await RideDet.updateOne(
          { ride_id: findrides[0]["ride_id"] },
          { $set: { commuter_id: commuter_id_new, seat_available: seats_avaialble, price: price_comm} }
        ).exec();
        findrides[0]["commuter_id"] = commuter_id_new;
        findrides[0]["price"] = price_comm;
        console.log('findrides',findrides);
        return {
